...
// YOU NEED TO DO A BETTER JOB OF CLEANUP HERE!! WHERE IS THE LIST ABOVE CLEANED UP?
navigation::~navigation(void) {
...
void navigation::setup(void) {
	...
	screen =  (displayObj*) new MSP3526_T(SCREEN_CS,SCREEN_RST);	// Create the screen (Display).
	if (screen) {																	// If we got  screen..
		if (screen->begin()) {													// Give the display a kick to get going.														
			screen->setRotation(PORTRAIT);									// Rotate correctly..
			ourNavDisp.setup();													// Call the display's setup function.
			success = true;
		}
	}
	if (!success) {
		Serial.println(F("navigation::setup() failed."));
		while(true);
	}
}
...
void navigation::printHelp(void) {
	...
	Serial.println(F("           ----------------------------------------------------------------------"));
	Serial.println(F("Pos         Shows us our current GPS position."));
	Serial.println(F("COG         Shows us our current course over ground."));
	...
	Serial.println(F("UTC         Get time delta from UTC we are using. Or, if a parameter is added, set it."));
	Serial.println(F("mCorrect    Get or set correction value from true to magnetic course."));
	Serial.println(F("spew			 Just spew toggles GPSvdata spewing. Adding on or off works too."));
}
...
void navigation::doUTC(void) {
	...
	if (cmdParser.numParams()==0) {								// If we're looking at no params..
		Serial.print(F("Time offset from UTC : "));
